...
- If raw export remains after the discharge reductions that can actually be applied in the current run, charge-capable batteries can absorb that remaining raw export temporarily. With a responsive charge-capable battery at `10 s` or less, this opportunistic charge path can start from the internal `10 W` target delta instead of waiting for the global `60 W` command deadband.
...
- When strong raw export continues while a responsive charge-capable battery is still physically discharging far above its newly lower allocation, the blueprint applies a temporary latency brake and can reduce that battery's discharge target below the stable allocation, down to `0 W`. This trades a possible short import rebound for less export while slow integrations apply the command.
