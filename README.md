...
- `Actual power sensor`: optional power sensor that reports the battery's real output. The blueprint keeps using the target entity and the house meter as its primary control loop, then uses this sensor only as a delayed secondary correction when the reading stays fresh and a non-priority battery is still delivering materially less than its active target after the response grace period.
