...
- `Night charge actions`: optional hooks executed when off-peak periods become active, and after Home Assistant start or automation reload if an off-peak period is already active. Use them to put the battery in its overnight charging mode.
...
- Optional charge and discharge actions only run while the battery is active in the matching direction. Night charge actions run only at off-peak start, or after start/reload while off-peak is already active. They are useful for integrations that still need a `select`, an auxiliary service call, or a helper-to-vendor translation layer.
