...
Localized Home Assistant blueprint project for steering up to four batteries from a single house power sensor. The blueprint allocates discharge, can absorb real export through opportunistic charging, stops managed discharge during off-peak periods, and writes a signed power target for each battery into a numeric entity. Optional custom actions remain available for integrations that also need a separate mode or service call. Each battery is grouped in its own collapsed section by default to keep the form compact.
...
- `Active off-peak entities`: optional list of `binary_sensor`, `input_boolean`, or `schedule` entities. If at least one selected entity stays `on` for 15 seconds, the blueprint stops commanding discharge and runs the configured night charge actions. Leaving `on` is handled immediately. This field replaces the old `Blocking entities` field and intentionally breaks its old semantics.
...
- `Night charge actions`: optional hooks executed first when off-peak periods become active, and after Home Assistant start or automation reload if an off-peak period is already active. Use them to put the battery in its overnight charging mode. If the sequence must keep running after later house power updates restart the automation, call a script with `script.turn_on` from these actions.
...
- During active off-peak periods, the blueprint never commands discharge. It returns managed discharge targets to neutral and runs each configured `Night charge actions` sequence after the off-peak entity has stayed `on` for 15 seconds.
...
- Off-peak entity transitions are treated as explicit control events: they can force the required sign change immediately so battery status actions are not left waiting behind a normal command cooldown.
...
- Validation now runs before any per-battery write or custom action. Every non-zero command is rechecked against the current house sensor state right before execution, and active off-peak periods force `off_peak_charge` before any discharge logic.
...
- Optional charge and discharge actions only run while the battery is active in the matching direction. Night charge actions run before the normal per-slot work and only at off-peak start, or after start/reload while off-peak is already active. They are useful for integrations that still need a `select`, an auxiliary service call, a `script.turn_on` handoff, or a helper-to-vendor translation layer.
