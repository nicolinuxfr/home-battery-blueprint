...
- If a battery has an active discharge target but fresh telemetry shows it is effectively no longer delivering after the response grace period, that battery is temporarily treated as unavailable for discharge. This stops an empty or self-protected battery from keeping a stale discharge target; the run log exposes this as `discharge_delivery_stalled`.
