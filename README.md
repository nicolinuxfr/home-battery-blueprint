...
- During discharge it allocates `max(house_power, 0)` across batteries, prioritizing flagged batteries first and then sorting by highest state of charge. If a priority battery is still ramping and reports fresh actual power, a lower-priority battery with a shorter cooldown can temporarily help until that priority battery catches up.
