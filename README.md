...
- When a battery has just received a new target, the blueprint now keeps that requested power reserved for the duration of its response grace period, while reconstructing house demand from measured battery output when fresh telemetry is available. This prevents a slower priority battery from immediately causing the same missing power to be reassigned to another battery.
