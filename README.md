...
- `Maximum discharge power` and `Maximum charge power`: manual limits used by the allocator. Charge-capable batteries can absorb real export while discharge targets are being reduced, as long as they no longer receive a discharge target in the current run.
- `Priority on discharge`: prioritized batteries discharge first; opportunistic charging prefers non-priority batteries first. Without any extra option, a battery that reaches `90%` state of charge is also treated as a stronger temporary priority so the allocator starts creating headroom before `100%`; while that battery stays active, this temporary priority is only released below `80%`.
...
- `Actual power sensor`: optional power sensor that reports the battery's real output. The blueprint keeps using the target entity and the house meter as its primary control loop, then, after the response grace period, uses the latest valid non-stale reading to rebuild the battery's real contribution instead of reserving the old target indefinitely. During discharge, fresh telemetry from a priority battery can also confirm that it is still ramping so a lower-priority battery with a shorter cooldown may keep helping temporarily until the priority battery catches up. A configured actual power sensor that becomes non-numeric or has not updated for more than 300 seconds is treated as stale and no longer lets an old target reserve discharge indefinitely.
- `Command cooldown`: per-battery delay between active target updates, and also the expected reaction window used by the discharge allocator. Lower values are treated as more reactive. Set it to `0` to disable the delay entirely. Writing `0`, flipping the sign, and starting from `0` when the raw house meter confirms matching import or export still happen immediately so the blueprint can stop, reverse, or resume a useful battery without waiting. While one battery is cooling down, the allocator keeps its current active target reserved when that battery is reached in priority order; a lower-priority cooldown no longer blocks a higher-priority or higher-SOC battery from taking the next allocation. A lower-priority battery with a shorter cooldown may temporarily help during discharge until the slower priority battery catches up, but only while the raw house meter still shows meaningful import.
...
- During discharge it allocates `max(house_power, 0)` across batteries, prioritizing flagged batteries first and then sorting by highest state of charge. Cooldown reservations are applied inside that same priority order, so an already-active lower-priority battery does not take demand before a higher-priority or higher-SOC battery. Independently from the user-facing priority flag, a battery that reaches `90%` state of charge is promoted into a stronger temporary priority tier so the allocator creates headroom before the battery hits `100%`; while that battery stays active, this promotion is only released below `80%`. If a priority battery is still ramping and reports fresh actual power, a lower-priority battery with a shorter cooldown can temporarily help until that priority battery catches up, but only while the raw house meter still shows at least `60 W` of import.
- When that temporary near-full priority battery is the one still ramping, the helper path becomes intentionally more permissive: an unlocked non-priority battery may fill the gap even if its configured cooldown is not shorter. This keeps a responsive helper battery such as a Zendure from waiting behind the protected battery, but the helper path is disabled as soon as the raw house meter stops showing meaningful import.
...
- When an optional actual power sensor is configured, the blueprint uses it after the response grace period to notice that a battery is materially under-delivering compared with its active target. This correction never replaces the primary target-plus-house-meter control loop. During discharge, fresh telemetry still drives priority-battery ramp assistance, but a merely valid non-stale reading is now enough to avoid overestimating an old target.
- If a battery has an active discharge target but valid non-stale telemetry shows it is effectively no longer delivering after the response grace period, or its configured actual power sensor becomes non-numeric or has stayed stale for more than 300 seconds, that battery is temporarily treated as unavailable for discharge allocation. It still remains commandable so the blueprint can write a stop target immediately. This stops an empty, self-protected, or no-longer-reporting battery from keeping a stale discharge target; the run log exposes this as `actual_power_usable`, `actual_power_stale`, `discharge_delivery_stalled` and `discharge_available`.
...
- When a battery has just received a new target, the blueprint keeps that requested power reserved for the duration of its response grace period once that battery is reached in priority order, while reconstructing house demand from measured battery output when fresh telemetry is available. If a slower priority battery is still ramping during that cooldown-derived reaction window, the allocator can also let a lower-priority battery with a shorter cooldown temporarily cover the observed gap until the priority battery catches up. That temporary assist is capped by the raw house import and is not created during export or inside the neutral deadband.
...
- The same raw-export trim also applies while holding an existing discharge target in the neutral deadband. As soon as the raw house meter shows at least `80 W` of real export during discharge, the blueprint can temporarily use charge-capable batteries that are not still assigned a discharge target in that run to absorb the spike while the other targets ramp down, preferring the shortest command cooldown first.
- To monitor regressions in that temporary assist, the blueprint keeps firing a `home_battery_blueprint_assist_overshoot` event if assist power ever remains active while the raw house meter already reports meaningful export. This should normally stay quiet; the event includes the current house power, reconstructed demand, assist watts, helper slots, priority slots still ramping, discharge targets, and actual battery discharge readings.
...
- The target written by the blueprint is signed: positive for discharge, negative for charge, `0` for neutral. Writing `0`, reacting to an invalid sensor, honoring a blocking entity, flipping the sign, or starting from `0` while the raw house meter confirms the matching grid flow all happen immediately without waiting for the cooldown. During an active cooldown, the blueprint still reserves the already-commanded power on that battery by default when its priority turn is reached, but a significant raw export now allows a same-direction discharge reduction before the cooldown expires.
