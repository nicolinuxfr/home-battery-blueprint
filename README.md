...
- During discharge, active targets are kept as the stable base first. Any remaining demand is assigned to regular batteries with longer command cooldowns first, so stable batteries carry the base load while responsive batteries remain available for residual corrections. High-SoC batteries can still be promoted ahead of that order to create headroom.
...
- Long-cooldown batteries no longer take raw-export reductions or discharge increases while a responsive charge-capable battery has room to absorb that export. The blueprint leaves slower batteries stable and lets responsive batteries reduce discharge or absorb the residual export by charging first; if opportunistic charging is already active and raw export still remains, slower discharge targets can be reduced as well to avoid charging and discharging at the same time while still exporting.
