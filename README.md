...
- If reallocation asks one battery to increase while another battery still needs to decrease but remains locked by cooldown, the increase waits until that decrease can be written. The blueprint prefers a short grid import over an export spike caused by temporarily incompatible targets.
- The same raw-export trim also applies while holding an existing discharge target in the neutral deadband. If reducing every discharge target still leaves at least the `80 W` release threshold of raw export, the blueprint can temporarily use charge-capable batteries that are not still assigned a discharge target in that run to absorb the residual spike, preferring the shortest command cooldown first.
