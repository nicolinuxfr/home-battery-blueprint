...
- The same raw-export trim also applies while holding an existing discharge target in the neutral deadband. If reducing every discharge target still leaves at least `500 W` of raw export beyond the release threshold, the blueprint can temporarily use charge-capable batteries that are not still assigned a discharge target in that run to absorb the residual spike, preferring the shortest command cooldown first.
