...
- If an off-peak entity briefly leaves `on` and returns within two minutes, the blueprint resumes off-peak charging immediately instead of waiting for the staggered startup delay again.
- While off-peak charging is active, a battery that was briefly returned to `0` can resume charging immediately instead of waiting for its command cooldown.
- The automation also rechecks its state when Home Assistant starts or automations are reloaded, so an already-active off-peak period is picked up after a blueprint import, automation edit, or restart.
