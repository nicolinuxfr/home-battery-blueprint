...
- Raw-export accounting now credits only discharge power that can disappear immediately from a responsive battery. Reducing a slow battery target still happens, but any export that may remain while that slow device catches up is kept available for emergency charging on a responsive charge-capable battery.
