...
- `Active off-peak entities`: optional list of `binary_sensor`, `input_boolean`, or `schedule` entities. If at least one selected entity stays `on` for 15 seconds, the blueprint stops normal target control and runs the configured night charge actions. Leaving `on` is handled immediately. This field replaces the old `Blocking entities` field and intentionally breaks its old semantics.
...
- During active off-peak periods, the blueprint never runs normal target control. It emits the run log, runs each configured `Night charge actions` sequence after the off-peak entity has stayed `on` for 15 seconds, then stops before validation, target writes, and charge/discharge hooks.
...
- Off-peak entity transitions are treated as explicit control events: they bypass the small-delta guard, run the configured night charge actions, and then stop before normal target control.
...
- Validation now runs before any normal per-battery write or charge/discharge custom action. Every non-zero normal command is rechecked against the current house sensor state right before execution; active off-peak periods run their night charge actions first and stop before this normal control path.
...
- Optional charge and discharge actions only run while the battery is active in the matching direction and only outside off-peak periods. Night charge actions run after the run log and only at off-peak start, or after start/reload while off-peak is already active. They are useful for integrations that need a `select`, an auxiliary service call, a `script.turn_on` handoff, or a helper-to-vendor translation layer.
