...
- En décharge, il répartit `max(house_power, 0)` entre les batteries, en privilégiant d'abord les batteries marquées prioritaires puis en triant par pourcentage de charge décroissant. Les réservations de cooldown sont appliquées dans ce même ordre de priorité, donc une batterie moins prioritaire déjà active ne capte plus la demande avant une batterie plus prioritaire ou avec un SOC plus haut. Indépendamment du flag visible dans le formulaire, une batterie qui atteint `90 %` de charge est promue dans un niveau de priorité temporaire plus fort afin de recréer une marge avant `100 %` ; tant qu'elle reste active, cette promotion n'est relâchée qu'en dessous de `80 %`. Si une batterie prioritaire est encore en phase de montée et remonte une puissance réelle fraîche, une batterie moins prioritaire avec un cooldown plus court peut aider temporairement jusqu'à ce que la batterie prioritaire rattrape sa consigne.
- Quand cette batterie presque pleine temporairement prioritaire est justement celle qui est encore en phase de montée, le chemin d'assistance devient volontairement plus permissif : une batterie non prioritaire disponible peut combler le trou même si son cooldown configuré n'est pas plus court. Cela évite qu'une batterie d'appoint réactive, comme une Zendure, reste à attendre derrière la batterie protégée alors que l'export commence déjà à apparaître.
...
- Cette réduction sur export brut s'applique aussi quand une consigne de décharge est maintenue dans la zone morte neutre. Si toutes les consignes de décharge ont été réduites et qu'il reste encore au moins `500 W` d'export brut au-delà du seuil de relâchement, le blueprint peut utiliser temporairement les batteries capables de charger qui n'ont plus de consigne de décharge dans ce run pour absorber le pic résiduel, en privilégiant le cooldown de commande le plus court.
