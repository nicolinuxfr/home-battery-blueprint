...
- Une bande morte interne fixe de `60 W` filtre les très petites oscillations du compteur, et un seuil interne fixe de relâchement à `80 W` maintient une batterie lente déjà active tant que le flux opposé reste faible. Si une batterie de décharge active a le cooldown le plus court et que ce cooldown est inférieur ou égal à `10 s`, elle peut quand même réduire l'export brut restant dans cette bande de relâchement afin que les batteries réactives gardent la maison plus proche de zéro sans faire bouger les batteries plus lentes. Une fois le cooldown propre à une batterie écoulé, un export brut plus faible à l'intérieur de cette bande peut aussi réduire cette consigne de décharge active au lieu de conserver un dépassement connu. Séparément, la batterie de décharge ou de charge disponible la plus courte à `10 s` ou moins peut recevoir de petites consignes résiduelles non nulles sous la bande morte principale, même si elle est actuellement au repos.
- Quand un export brut fort continue alors qu'une batterie réactive capable de charger débite encore physiquement bien au-dessus de sa nouvelle allocation réduite, le blueprint applique un frein de latence temporaire et peut abaisser la consigne de décharge de cette batterie sous l'allocation stable, jusqu'à `0 W`. Cela accepte un possible court rebond en import pour réduire l'export pendant que les intégrations lentes appliquent la commande.
...
- La batterie réactive disponible au cooldown le plus court peut recevoir de petites consignes non nulles jusqu'à la bande interne de `10 W` afin d'effacer le résiduel restant au lieu d'attendre la bande morte globale de `60 W`. Cela s'applique aux consignes de décharge et de charge disponibles, même quand la batterie est actuellement au repos. Si l'entité de consigne expose un minimum positif supérieur à cette petite consigne, le blueprint borne la valeur écrite à ce minimum.
