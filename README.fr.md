...
- `Capteur de puissance réelle` : capteur optionnel qui remonte la puissance réellement fournie par la batterie. Le blueprint continue d'utiliser l'entité de consigne et le compteur maison comme boucle principale. Après le délai de réponse, une mesure valide non périmée sert seulement de garde-fou pour éviter de compter une batterie comme apport réel quand elle ne délivre plus. Un capteur de puissance réelle configuré qui devient non numérique ou qui ne s'est pas mis à jour depuis plus de 300 secondes est considéré comme périmé ; cette batterie est exclue des allocations de décharge additionnelles jusqu'au retour de la télémétrie, tandis qu'une consigne déjà active peut rester en place sauf arrêt, changement de signe ou export réel imposant une correction.
...
- Si une batterie garde une consigne de décharge active mais que sa télémétrie valide non périmée montre qu'elle ne délivre pratiquement plus rien après le délai de réponse, ou que son capteur de puissance réelle configuré devient non numérique ou reste périmé plus de 300 secondes, elle est temporairement exclue des allocations de décharge additionnelles. Sa consigne active peut rester maintenue pour la stabilité, mais sa contribution mesurée n'est pas comptée, ce qui laisse les batteries plus rapides couvrir le résiduel. Elle reste commandable pour que le blueprint puisse écrire immédiatement un arrêt, un changement de signe ou une réduction imposée par un export réel. L'événement de run expose cet état avec `actual_power_usable`, `actual_power_stale`, `discharge_delivery_stalled` et `discharge_available`.
