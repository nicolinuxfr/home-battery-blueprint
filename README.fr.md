...
- Si une batterie garde une consigne de décharge active mais qu'une télémétrie fraîche montre qu'elle ne délivre pratiquement plus rien après le délai de réponse, ou que son capteur de puissance réelle configuré reste périmé plus de 300 secondes, elle est temporairement traitée comme indisponible pour l'allocation de décharge. Elle reste tout de même commandable pour que le blueprint puisse écrire immédiatement une consigne d'arrêt. Cela évite qu'une batterie vide, autoprotégée ou qui ne remonte plus de mesure conserve une consigne de décharge obsolète ; l'événement de run expose cet état avec `actual_power_stale`, `discharge_delivery_stalled` et `discharge_available`.
