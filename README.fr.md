...
- Si une batterie garde une consigne de décharge active mais qu'une télémétrie fraîche montre qu'elle ne délivre pratiquement plus rien après le délai de réponse, elle est temporairement traitée comme indisponible pour l'allocation de décharge. Elle reste tout de même commandable pour que le blueprint puisse écrire immédiatement une consigne d'arrêt. Cela évite qu'une batterie vide ou autoprotégée conserve une consigne de décharge obsolète ; l'événement de run expose cet état avec `discharge_delivery_stalled` et `discharge_available`.
