...
- `Capteur de puissance réelle` : capteur optionnel qui remonte la puissance réellement fournie par la batterie. Le blueprint continue d'utiliser l'entité de consigne et le compteur maison comme boucle principale. Après le délai de réponse, une mesure valide non périmée sert seulement de garde-fou pour éviter de réserver une ancienne consigne d'une batterie qui ne délivre plus réellement. Un capteur de puissance réelle configuré qui devient non numérique ou qui ne s'est pas mis à jour depuis plus de 300 secondes est considéré comme périmé ; cette batterie est exclue des nouvelles allocations de décharge jusqu'au retour de la télémétrie, tout en permettant encore l'écriture d'une consigne d'arrêt.
...
- Si une batterie garde une consigne de décharge active mais que sa télémétrie valide non périmée montre qu'elle ne délivre pratiquement plus rien après le délai de réponse, ou que son capteur de puissance réelle configuré devient non numérique ou reste périmé plus de 300 secondes, elle est temporairement traitée comme indisponible pour l'allocation de décharge même si sa consigne courante est déjà retombée à `0`. Elle reste tout de même commandable pour que le blueprint puisse écrire immédiatement une consigne d'arrêt. Cela évite qu'une batterie vide, autoprotégée ou qui ne remonte plus de mesure capte une nouvelle demande de décharge ou conserve une consigne de décharge obsolète ; l'événement de run expose cet état avec `actual_power_usable`, `actual_power_stale`, `discharge_delivery_stalled` et `discharge_available`.
