...
- `Prioritaire en décharge` : les batteries prioritaires se vident d'abord ; la charge opportuniste préfère d'abord les batteries non prioritaires. Sans option supplémentaire, une batterie qui atteint `90 %` de charge est aussi traitée comme temporairement prioritaire pour recréer une marge avant `100 %` ; tant qu'elle reste active, cette priorité temporaire n'est relâchée qu'en dessous de `80 %`.
...
- En décharge, il répartit `max(house_power, 0)` entre les batteries, en privilégiant d'abord les batteries marquées prioritaires puis en triant par pourcentage de charge décroissant. Indépendamment du flag visible dans le formulaire, une batterie qui atteint `90 %` de charge est aussi promue temporairement afin de recréer une marge avant `100 %` ; tant qu'elle reste active, cette promotion n'est relâchée qu'en dessous de `80 %`. Si une batterie prioritaire est encore en phase de montée et remonte une puissance réelle fraîche, une batterie moins prioritaire avec un cooldown plus court peut aider temporairement jusqu'à ce que la batterie prioritaire rattrape sa consigne.
