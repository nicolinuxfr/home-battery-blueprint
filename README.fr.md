...
- `Puissance maximale de charge batteries en heures creuses` : plafond total de charge batterie en watts. Le blueprint peut allouer jusqu'à cette puissance aux batteries gérées, indépendamment de la consommation actuelle de la maison. Mets `0 W` pour désactiver la charge heures creuses.
...
- `État de charge minimum pour décharger` : seuil de réserve sous lequel le slot est retiré de l'allocation de décharge et toute consigne de décharge active revient à `0`. La valeur par défaut est `15 %`.
...
- `Batterie réactive` : active cette option pour rendre la batterie réactive. Elle sera ajustée fréquemment pour absorber les variations résiduelles de demande et récupérer l'export dès que possible. Laisse cette option désactivée (par défaut) pour un comportement stable où la batterie garde une consigne constante et laisse les batteries réactives gérer les fluctuations.
...
- `L'entité de consigne utilise des valeurs absolues` : active cette option quand l'entité de consigne attend une puissance positive en charge comme en décharge. Continue à utiliser les actions de charge/décharge pour choisir le sens de fonctionnement de la batterie.
- `Capteur de puissance réelle` : capteur optionnel qui remonte la puissance réellement fournie par la batterie. Le blueprint continue d'utiliser l'entité de consigne et le compteur maison comme boucle principale. Après le délai de réponse, une mesure valide non périmée sert seulement de garde-fou pour éviter de compter une batterie comme apport réel quand elle ne délivre plus. Une mesure périmée n'empêche pas une batterie au repos de démarrer et ne suffit plus à marquer une batterie active comme bloquée ; l'allocateur retombe sur la consigne demandée jusqu'au retour d'une mesure valide.
- `Cooldown de commande` : délai par batterie entre deux mises à jour actives de consigne. Un cooldown court fonctionne mieux avec les batteries réactives pour absorber rapidement les variations résiduelles. Un cooldown plus long maintient la consigne plus stable. Mets `0` pour supprimer complètement ce délai. Les passages à `0` restent immédiats pour permettre un arrêt de sécurité, mais les batteries non réactives ne basculent plus directement entre charge et décharge et ne redémarrent plus depuis `0` avant la fin de leur cooldown. Un export brut peut aussi réduire une consigne de décharge active avant la fin du cooldown.
- `Actions de décharge` et `Actions de charge` : hooks optionnels exécutés à chaque mise à jour active dans le sens correspondant, puis à nouveau quand le blueprint doit arrêter une batterie qui débite ou charge encore physiquement. Ils reçoivent des variables d'exécution comme `battery_slot`, `battery_soc`, `target_power_w`, `target_discharge_w`, `target_charge_w`, `house_power_w` et `export_surplus_w` ; les variables de consigne sont initialisées globalement puis remplacées par les valeurs du slot courant avant chaque séquence d'actions personnalisées.
...
- Pendant les heures creuses actives, le blueprint ne commande aucune décharge. Si la prévision solaire, la capacité utile ou la limite de puissance de charge batterie ne permettent pas de charger, il passe en `off_peak_idle` et remet les batteries gérées à neutre.
...
- Si une entité heures creuses quitte brièvement l'état `on` puis y revient sous deux minutes, le blueprint reprend immédiatement la charge heures creuses au lieu d'attendre à nouveau le délai de démarrage échelonné.
- Pendant une charge heures creuses active, une batterie brièvement remise à `0` peut reprendre la charge immédiatement sans attendre son cooldown de commande.
- L'automatisation réévalue aussi son état au démarrage de Home Assistant ou quand les automatisations sont rechargées, afin de reprendre une période heures creuses déjà active après un import de blueprint, une modification d'automatisation ou un redémarrage.
- La limite de puissance de charge heures creuses ne concerne que la charge des batteries gérées. Elle n'est pas réduite par la consommation actuelle de la maison.
...
- En décharge, les consignes déjà actives forment d'abord la base stable. Le résiduel est ensuite donné aux batteries non réactives en premier, afin que les batteries stables portent la charge de base pendant que les batteries réactives restent disponibles pour les corrections résiduelles. Les batteries à SoC élevé peuvent toujours être promues avant cet ordre pour créer de la marge.
...
- Quand la demande baisse, la décharge est réduite dans l'ordre inverse : les batteries au cooldown le plus court sont relâchées d'abord. Si un export réel reste présent, les batteries capables de charger et au cooldown court peuvent absorber temporairement cet excès au lieu d'attendre que les intégrations lentes acceptent leur prochaine consigne. Une batterie qui décharge encore physiquement ne bascule plus directement en charge d'urgence ; elle doit d'abord revenir dans la zone morte de décharge.
- L'allocateur reconstruit la demande maison sous-jacente à partir du capteur maison net en réajoutant la contribution effective de chaque batterie gérée : la consigne active quand aucun capteur de puissance réelle n'est configuré, pendant la fenêtre de réponse ou quand la télémétrie est périmée, puis la dernière puissance mesurée valide quand elle est exploitable. Une batterie qui continue à débiter ou charger physiquement après une consigne revenue à `0` est aussi comptée et réservée dans l'allocation suivante, pour éviter que le blueprint ajoute la même puissance sur une autre batterie. Une télémétrie configurée mais non numérique ou très périmée ne retire donc plus à elle seule une batterie active du plan.
...
- Les batteries non réactives ne prennent plus les réductions sur export brut ni les augmentations de décharge tant qu'une batterie réactive capable de charger a encore de la marge pour absorber cet export. Le blueprint garde les batteries stables intouchées et laisse d'abord les batteries réactives réduire leur décharge ou absorber l'export résiduel en charge ; si la charge opportuniste est déjà active et que l'export brut persiste, les consignes de décharge stables peuvent aussi être réduites pour éviter de charger et décharger en même temps tout en continuant d'exporter.
...
- La bande morte de commande et le seuil de relâchement internes valent tous les deux `10 W`, ce qui permet aux batteries réactives de corriger les petites variations d'import/export au lieu de laisser un résiduel volontaire plus large. La batterie réactive au cooldown le plus court reste prioritaire pour ces corrections, afin de garder les batteries stables plus stables quand une batterie réactive peut absorber la variation.
...
- Quand la consigne signée calculée s'arrondit à la même valeur que celle déjà présente sur l'entité de consigne, le blueprint saute désormais cette écriture redondante et ne relance pas non plus les actions personnalisées dans le même sens. Cela réduit les appels de service sans effet et les effets de bord dupliqués côté intégrations, tout en conservant les vrais changements de consigne et les arrêts forcés.
- La consigne écrite par le blueprint est signée : positive en décharge, négative en charge, `0` en neutre. Un passage à `0`, un capteur invalide ou une entrée en heures creuses peut toujours arrêter une batterie immédiatement sans attendre le cooldown. Les inversions directes de signe et les redémarrages depuis `0` respectent maintenant le cooldown pour les batteries non réactives, afin que les batteries à long cooldown restent stables pendant que les batteries à cooldown court absorbent les variations temporaires d'import/export. Pendant un cooldown actif, le blueprint réserve toujours par défaut la puissance déjà commandée sur cette batterie, mais un export brut autorise une baisse de consigne de décharge avant la fin du cooldown.
- Les inversions directes de signe attendent maintenant le délai de réponse batterie, pas seulement le cooldown de commande. Cela évite qu'une batterie à cooldown court alterne charge et décharge avant que la commande précédente ait eu le temps d'apparaître dans la télémétrie.
- Les transitions des entités heures creuses sont traitées comme des événements de pilotage explicites : elles peuvent forcer immédiatement le changement de signe nécessaire pour que les actions de statut batterie ne restent pas bloquées derrière un cooldown normal.
