...
- Si une batterie garde une consigne de décharge active mais qu'une télémétrie fraîche montre qu'elle ne délivre pratiquement plus rien après le délai de réponse, elle est temporairement traitée comme indisponible en décharge. Cela évite qu'une batterie vide ou autoprotégée conserve une consigne de décharge obsolète ; l'événement de run expose cet état avec `discharge_delivery_stalled`.
